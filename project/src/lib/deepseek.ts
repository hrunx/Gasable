...
export async function talkToDeepSeek(prompt: string): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      return data.content || responseText;
    } catch (parseError) {
      console.warn('[DeepSeek] Failed to parse response as JSON:', parseError);
      // Return the raw text - the calling function will handle JSON extraction if needed
      return responseText;
    }
  } catch (error) {
