...
interface ProductSetupProgressProps {
  currentStep: number;
...
const subSteps = {
  ...
  'shipment-type': { label: 'Shipment Method' },
  'delivery-zones': { label: 'Delivery Zones' },
};

const ProductSetupProgress: React.FC<ProductSetupProgressProps> = ({ currentStep, currentSubStep }) => {
  // Determine which set of sub-steps to show based on the current main step
  const getRelevantSubSteps = () => {
    if (currentStep === 1) { // Add Product
      return [
    ...
    } else if (currentStep === 2) { // Shipment
      return [
...
export default ProductSetupProgress;